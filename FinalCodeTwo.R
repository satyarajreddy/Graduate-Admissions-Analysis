...
setwd("C:/Users/kumar/Desktop/Big Data Term project")
...
ggplot(grad.df,aes(gre,admit))+geom_point()+geom_smooth()+ggtitle("GRE Scores vs Admit Chance")



ggplot(grad.df,aes(toefl,admit))+geom_point()+geom_smooth()+ggtitle("TOEFL Scores vs Admit Chance")
...
hist(grad.df$gre,col="#0099FF",
     xlab="GRE Scores",
     ylab="Frequency",
     main="Histogram for GRE Scores",
     labels=TRUE,
     ylim=c(0,100))

hist(grad.df$toefl,col="#0099FF",
     xlab="TOEFL Scores",
     ylab="Frequency",
     main="Histogram for TOEFL Scores",
     labels=TRUE,
     ylim=c(0,100))

#We can see from above plots that the median of GRE is around 318. Also, one should reach 
#325 if he/she wants to be top 25%.

ggplot(grad.df,aes(gpa,admit))+geom_point(color="#339966")+facet_grid(research~.)

# I want to research the correlation between GPA and admission rate. I find useful to divide 
#it into two groups- students with research experience and students who do not. When compared to students wit research expience, the other group of students barely get 9+ gpa. If you did research before and you gpa is higher than 9, we can be pretty sure you have a BIG chance to be admitted.
...
set.seed(30)
new <- createDataPartition(y = grad.df$chance_low_high,p = 0.80,list = FALSE) #creating train and test dataset
...
set.seed(2)
training<-sample(row.names(grad_nn.df), dim(grad_nn.df)[1]*0.8)
